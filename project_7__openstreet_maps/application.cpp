...
// this functions build the graph by adding the footways as edges to the graph
...
void readNodesToGraph(graph<long long, double> &G, map<long long, Coordinates> Nodes) {
    for (auto &e : Nodes) {
        G.addVertex(e.first);
    }
}

bool checkBuildingExists (string buildingName, BuildingInfo & build, vector <BuildingInfo> Buildings) {
     // bool check = false;
    ...
    // checking for the full name, if it exists
    for (auto e : Buildings) {
